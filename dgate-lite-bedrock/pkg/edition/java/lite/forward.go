...
func findRoute(
	...
	if route.Protocol == "udp" {
		return log.V(1), src, nil, nil, fmt.Errorf("route for host %s is UDP-only (Bedrock), but this is a TCP (Java) connection", clearedHost)
	}
...
func dialRoute(
	...
	// Use ProxyProtocol for TCP connections (explicit or automatic for "any" protocol)
	useProxyProtocol := route.ProxyProtocol || (route.Protocol == "any")
	if useProxyProtocol {
